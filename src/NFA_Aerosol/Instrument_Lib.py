...
def Load_OPS(file, start=0, end=0):
        ...
        # Load particle data from the txt file
        OPS = pd.read_csv(file, sep=',', header=13, encoding= 'latin1')
...
def Load_OPS_Direct(file, start=0, end=0):
        ...
        Bin_edges = np.genfromtxt(file,delimiter=",",skip_header=11,skip_footer=OPS.shape[0]+10,usecols=1)
        
        # Calculate mid points of the sizebins
        Bin_mids = Bin_edges[:-1] + (Bin_edges[1:]-Bin_edges[:-1])/2
        
        # Load the start date and time from the datafile
        OPS_date = np.genfromtxt(file,delimiter=",",skip_header=7,skip_footer=OPS.shape[0]+30,dtype="str")[1]
        OPS_starttime = np.genfromtxt(file,delimiter=",",skip_header=6,skip_footer=OPS.shape[0]+31,dtype="str")[1]
